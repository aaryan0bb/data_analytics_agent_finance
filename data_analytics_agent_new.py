...
TAVILY_TOOL_DEF = [{
    ...
    "parameters": {
        ...
        "required": ["query", "max_results", "search_depth", "include_answer"],
...
class Improvement(BaseModel):
    """Single actionable improvement proposed by the evaluator (simplified).

    Only two fields are required to avoid schema drift and parsing failures.
    """

    description: str = Field(..., min_length=5, description="Concrete improvement proposal")
    rationale: str = Field(..., min_length=10, description="Why this helps")
...
2) Produce a high-signal EvalReport that includes:
   - improvements: a list of objects with only {description, rationale}
...
When proposing web research queries, optimize for downstream computability.

Purpose:
- Your queries will feed an evidence pass (web search) and then be used to justify small, local actions and code changes.
- Therefore craft queries that lead to *verifiable, citable facts* or definitions we can act on (formulas, parameter ranges, canonical sources, licensing notes), not broad think-pieces.

Write 1–5 high-signal queries per theme. For each query, briefly state:
- what we’re trying to verify or extract (1 sentence),
- why it matters for this analysis (1 sentence).

Make the query strings specific enough to surface authoritative sources:
- Include key terms, symbols, time ranges, and nouns that disambiguate (“‘rolling beta’ definition site:papers.ssrn.com” beats “beta method”).
- Use recency filters only if the topic is time-sensitive; otherwise prefer evergreen references.
- Suggest 1–3 *include* domains (e.g., docs, regulators, data providers) and 0–3 *exclude* domains (e.g., wikis, forums) when it will improve precision.
- Cap desired results per query to a small number (3–7) to reduce noise.

Avoid:
- Opinions, blogspam, vendor fluff without data,
- queries that will only yield paywalled abstracts with no usable facts,
- vague prompts like “latest trends”, “insights”, “compare X and Y” without measurable targets.

Think: “If I run this query, can I extract a definition, a parameter, or a small set of citable facts that we can transform into code or metrics?”   

Strictly return ONLY JSON conforming to the EvalReport schema. For improvements, include only {description, rationale}. Do NOT include any extra text.
"""
...
- You must return ONLY the code to append between:
  # === DELTA v1 START ===
  # === DELTA v1 END ===
- you have freedom to Use existing variables/functions from the current code.
...
class AgentState(TypedDict):
    ...
    # Subpart 4: acceptance & rollback
    acceptance_result: Dict[str, Any]
    acceptance_failed: bool
    last_good_code: str
    last_good_manifest_summary: Dict[str, Any]
    last_good_iteration_index: int
...
def build_manifest_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        # Subpart 4: Acceptance evaluation & rollback handling
        try:
            dp = state.get("delta_plan", {}) or {}
            requested_cols = list(dp.get("new_columns", []) or [])
            requested_metrics = list((dp.get("add_metrics", {}) or {}).keys())
            acceptance_needed = bool(requested_cols or requested_metrics)

            # Compute presence in manifest
            present_cols = set()
            try:
                for t in getattr(manifest, "tables", []) or []:
                    for c in (getattr(t, "columns", []) or []):
                        present_cols.add(str(c))
            except Exception:
                pass
            cols_present_count = sum(1 for c in requested_cols if c in present_cols)

            metrics_dict = getattr(manifest, "metrics", {}) or {}
            metrics_present_count = 0
            for m in requested_metrics:
                v = metrics_dict.get(m)
                if isinstance(v, (int, float)) and not (isinstance(v, float) and (pd.isna(v) or np.isinf(v))):
                    metrics_present_count += 1

            acceptance_pass = True
            if acceptance_needed:
                if requested_cols and cols_present_count < len(requested_cols):
                    acceptance_pass = False
                if requested_metrics and metrics_present_count < 1:
                    acceptance_pass = False

            state["acceptance_result"] = {
                "requested_new_columns": requested_cols,
                "new_columns_present": int(cols_present_count),
                "requested_metrics": requested_metrics,
                "metrics_present": int(metrics_present_count),
                "passed": bool(acceptance_pass),
            }
            if not acceptance_needed or acceptance_pass:
                # Snapshot last known good for rollback safety
                state["last_good_code"] = state.get("generated_code", "") or ""
                state["last_good_manifest_summary"] = state.get("manifest_summary", {}) or {}
                state["last_good_iteration_index"] = int(state.get("iteration_index", 0) or 0)
                logger.info("Acceptance: PASS (or not needed). Last-good snapshot saved.")
            else:
                # Acceptance failed: rollback code/summary if snapshot exists
                state["acceptance_failed"] = True
                reason = []
                if requested_cols and cols_present_count < len(requested_cols):
                    reason.append(f"columns {cols_present_count}/{len(requested_cols)}")
                if requested_metrics and metrics_present_count < 1:
                    reason.append("metrics 0/required>=1")
                msg = "; ".join(reason) or "acceptance conditions not met"
                state["error_message"] = f"Acceptance failed: {msg}"
                roll_detail = {
                    "reason": msg,
                    "round": int(state.get("improvement_round", 0) or 0),
                    "acceptance_result": state["acceptance_result"],
                }
                if state.get("last_good_code"):
                    state["generated_code"] = state["last_good_code"]
                if state.get("last_good_manifest_summary"):
                    state["manifest_summary"] = state["last_good_manifest_summary"]
                state["rollback_info"] = roll_detail
                logger.warning(f"Acceptance: FAIL. Rolled back to last-known-good. Detail: {roll_detail}")
        except Exception as exc:
            logger.warning(f"Acceptance evaluation error (continuing): {exc}")
...
def extra_tool_exec_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        # Merit filter: only include tools that produce keys referenced by delta_plan
        dp = state.get("delta_plan", {}) or {}
        dp_keys = set(dp.get("uses_existing_keys", []) or [])
        if not dp_keys:
            logger.info("Extra tools merit filter: no delta_plan keys; skipping extra tools.")
            state["delta_tool_outputs"] = {}
            return state
        ...
        for suggestion in suggestions:
            try:
                ...
                try:
                    ...
                    sem_key = None
                if sem_key and sem_key in dp_keys and tool_name in registry_schemas:
                    if allowed >= MAX_ALLOWED:
                        logger.info(f"Extra tools merit filter: cap reached; skipping {tool_name}")
                        continue
                    ...
                    allowed += 1
                    logger.info(f"ALLOW extra tool {tool_name} (sem_key={sem_key}) via merit filter")
                else:
                    logger.info(f"SKIP extra tool {tool_name} (sem_key={sem_key}) not in delta_plan keys")
            except Exception as e:
                logger.warning(f"Suggestion parse failed; skipping: {e}")
                continue
...
def delta_code_gen_node(state: AgentState) -> AgentState:
    ...
    # Preflight: skip if plan empty or references unknown keys
    try:
        dp = state.get("delta_plan", {}) or {}
        empty_plan = (not dp.get("new_columns") and not dp.get("add_metrics") and not dp.get("add_figures"))
        if empty_plan:
            logger.info("Delta preflight: empty delta_plan; skipping delta update.")
            state["delta_tool_outputs"] = {}
            state["should_finalize_after_eval"] = True
            return state
        known_keys = set((state.get("tool_outputs", {}) or {}).keys())
        uses_keys = set(dp.get("uses_existing_keys", []) or [])
        unknown = sorted(list(uses_keys - known_keys))
        if unknown:
            try:
                import difflib
                suggestions = {u: difflib.get_close_matches(u, list(known_keys), n=2) for u in unknown}
            except Exception:
                suggestions = {}
            logger.warning(f"Delta preflight: unknown keys in delta_plan: {unknown}; suggestions: {suggestions}")
            state["delta_tool_outputs"] = {}
            state["should_finalize_after_eval"] = True
            return state
    except Exception as exc:
        logger.warning(f"Delta preflight failed (continuing conservatively): {exc}")
    payload = {
...
def post_manifest_router(state: AgentState) -> str:
    ...
    if not state.get("manifest_ok"):
        return "finalize"
    if state.get("acceptance_failed"):
        return "finalize"
...
class DataAnalyticsAgent:
    ...
    def process_request(self, user_input: str, task_description: str = None) -> str:
        ...
        initial_state = AgentState(
            ...
            last_good_code="",
            last_good_manifest_summary={},
            last_good_iteration_index=0,
            rollback_info={},
            # Unused tools tracking fields
            unused_tool_picks=[],
            unused_tool_files={},
...
if __name__ == "__main__":
  ...
  "Is insider buying a leading signal? Rank stocks by net insider buys in the last 90 days and run a 3-month forward performance test.please analyze the performance of your output and enhance the insider buying leading signal to build profitable strategy"
    )
